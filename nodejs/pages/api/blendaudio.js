...
export default async function handler(req, res) {
  ...
  const command = `ffmpeg -i ${audio1} -i ${audio2} -filter_complex "[1:a]adelay=0|0[volume];[volume]volume=0.05[a2];[0:a][a2]amix=inputs=2[aout]" -map "[aout]" -c:a mp3 -t $(ffprobe -i ${audio1} -show_entries format=duration -v quiet -of csv="p=0") ${outputPath}`;
