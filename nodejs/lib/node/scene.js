...
class FFScene extends FFCon {
  ...
  annotate() {
    let maxAbsEnd = Math.max(...this.allNodes.filter(x => !x.isVirtual && !x.flexibleDuration).map(x => x.realAbsEndTime));
    if (!maxAbsEnd) {
      maxAbsEnd = Math.max(...this.allNodes.filter(x => !x.isVirtual).map(x => x.realAbsEndTime));
    }
