...
var _generateMultiTextureShader = _interopRequireDefault(require("./generateMultiTextureShader"));
var _checkMaxIfStatmentsInShader = _interopRequireDefault(require("../../renderers/webgl/utils/checkMaxIfStatmentsInShader"));
var _BatchBuffer = _interopRequireDefault(require("./BatchBuffer"));
...
var TICK = 0;
var TEXTURE_TICK = 0;
class SpriteRenderer extends _ObjectRenderer.default {
  constructor(renderer) {
    super(renderer);
    this.vertSize = 5;
    this.vertByteSize = this.vertSize * 4;
    this.size = _settings.default.SPRITE_BATCH_SIZE; // 2000 is a nice balance between mobile / desktop

    // the total number of bytes in our batch
    // let numVerts = this.size * 4 * this.vertByteSize;

    this.buffers = [];
    for (var i = 1; i <= _bitTwiddle.default.nextPow2(this.size); i *= 2) {
      this.buffers.push(new _BatchBuffer.default(i * 4 * this.vertByteSize));
    }
     ...
     * @member {Uint16Array}
     */
    this.indices = (0, _createIndicesForQuads.default)(this.size);
  ...
  onContextChange() {
    ...
    if (this.renderer.legacy) {
      ...
      this.MAX_TEXTURES = Math.min(gl.getParameter(gl.MAX_TEXTURE_IMAGE_UNITS), _settings.default.SPRITE_MAX_TEXTURES);

      // step 2: check the maximum number of if statements the shader can have too..
      this.MAX_TEXTURES = (0, _checkMaxIfStatmentsInShader.default)(this.MAX_TEXTURES, gl);
    }
    this.shader = (0, _generateMultiTextureShader.default)(gl, this.MAX_TEXTURES);
    ...
    for (var i = 0; i < this.vaoMax; i++) {
      ...
      var vertexBuffer = this.vertexBuffers[i] = _pixiGlCore.default.GLBuffer.createVertexBuffer(gl, null, gl.STREAM_DRAW);
  ...
  flush() {
    ...
    var np2 = _bitTwiddle.default.nextPow2(this.currentIndex);
    var log2 = _bitTwiddle.default.log2(np2);
    ...
    currentGroup.size = i - currentGroup.start;
    if (!_settings.default.CAN_UPLOAD_SAME_BUFFER) {
      ...
      if (this.vaoMax <= this.vertexCount) {
        ...
        /* eslint-disable max-len */
        var vertexBuffer = this.vertexBuffers[this.vertexCount] = _pixiGlCore.default.GLBuffer.createVertexBuffer(gl, null, gl.STREAM_DRAW);
        /* eslint-enable max-len */
...
_WebGLRenderer.default.registerPlugin("sprite", SpriteRenderer);
